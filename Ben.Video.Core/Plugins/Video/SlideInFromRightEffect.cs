...
/// <summary>
/// Slides the content in from the right edge of the frame.
/// Inspired by animate.css <c>slideInRight</c>.
/// </summary>
public sealed class SlideInFromRightEffect : IClipEffect
{
    public string EffectId    => "slide_in_from_right";
    public string DisplayName => "Slide In From Right";
        ...
        int canvasWidth = 0, int canvasHeight = 0)
    {
        ...
        // Canvas = [black | image]. Crop window at x=iw*(1-ease) goes from iw→0.
        var ease = EasingHelper.GetClamped(eas, "t", d);
        var dStr = d.ToString("F3", ic);
        return $"pad=iw*2:ih:iw:0:black,crop=iw:ih:'if(lt(t,{dStr}),iw*(1-{ease}),0)':0";
    }
}
