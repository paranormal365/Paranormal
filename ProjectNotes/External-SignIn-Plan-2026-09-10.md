...
| E | **Merged** to develop `b0d4f9f1` and pushed; `EmailKind` migration applied to the testing copy `IsHauntedDb_player` with an explicit `--connection`. `20260910143508_AppUserEmailKind` **applied to production `IsHauntedDb`** the same day, on Ben's say-so, by an explicit `--connection` naming it; the listing afterwards shows nothing pending. Additive, so the deployed build kept working | Suite green on the merged tip; `migrations list` against production before and after |
