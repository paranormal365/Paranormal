...
> |---|---|---|
> | 1 | **OPEN** | the users grid offering Delete on your own row. Items 149/150 were about this shape, so it may already be gone — not yet verified |
> | 2 | **Closed 2026-09-22** | already fixed: `LoginAsync` breaks out on "Invalid email or password" and "locked". But the guard reads the PAGE, and nothing asserted the page still says those words — the only file mentioning the phrase outside `Login.razor` was the helper depending on it. `LoginWordingTheHarnessDependsOnTests` now pins both ends |
> | 3 | **OPEN** | two seeded password sets; `seeded-passwords.sh` documents the keys now, but this was not re-checked |
> | 4 | **OPEN** | a test enrolling the shared account in two-factor |
> | 5 | Closed | `OrganizationMembershipController` answers `Forbid()` without standing, and the subscription GET is gated by `OrganizationSettings.Read` |
> | 6 | Recorded, no action | what held up is worth keeping as written |
> | 7 | **Not triaged** | the "smaller things" were never separated out or checked |
>
> One correction to my own reading of #2, for whoever comes next: I first called it a production
> fault harming real users. It is not. It damages the SUITE — it locks shared seed accounts during
> a run — which is serious for trusting test results and not serious for anybody using the site.
