...
> **Where each of these ended up — added 2026-09-22, when this document was finally merged.**
>
> These sat on `docs/findings-sweep-20260904` for eighteen days, unmerged and not in the backlog,
> which is why several were fixed by accident rather than on purpose and two were never looked at.
> Merging the record is the point: a finding nobody can find is a finding nobody closes.
>
> | # | Where it stands | How I know |
> |---|---|---|
> | 1 | Fixed | said so here at the time, on a branch |
> | 2 | Fixed | same branch |
> | 3 | Closed | production never had a $0 band; the seeder no longer makes one |
> | 4 | **Closed 2026-09-22** | checked end to end: the client returns the sentence, the page prefers it, `RefusalReachesThePageTests` pins it. The structural half measured 183 error paths taking the server's words against 43 writing a literal; five more sites converted (roles ×3, CMS draft ×2), the rest checked and left because their endpoints send no prose |
> | 5 | Closed | `SignInLink.For` carries `returnUrl` |
> | 6 | **OPEN** | nothing on the calendar says how to add an event — not yet verified |
> | 7 | Closed | `Logout.razor` exists |
> | 8 | **Closed** — verified 2026-09-22 | `OrganizationSecurityController.UpsertMembership` now asks `WhyCannotAddMemberAsync`, and its comment cites this sweep. Worth noting it was never in the plan's triage table at all: it got fixed without the record being updated |
> | 9 | **Not triaged** | the "small things" were never separated out or checked |
