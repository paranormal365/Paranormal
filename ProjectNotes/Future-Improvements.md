...
>
> Found and flagged, not fixed (out of scope for this feature): `UploadFileController.GetAll` has no owner filter at all — any authenticated user can currently list every file in the system — and `Download` only checks `IsPublic`, with no ownership/sharing-audience check, so any authenticated (or even anonymous, for public files) user can download any file by ID. Together these bypass the entire Phase 1/2 sharing and comment-audience model. Spawned as a separate task rather than folded into this branch.
