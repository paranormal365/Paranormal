...
public static class AccountStorageGuard
{
    ...
    /// <summary>Bytes this account is storing on its own behalf.</summary>
    /// <remarks>
    /// Field-session files with no investigation are the whole of it today. Written as its own
    /// method so that when a second kind of personal upload appears, there is one place that
    /// decides what "their own storage" means rather than two queries that disagree.
    /// </remarks>
