...
// ValidationLink is set ONLY on the response to ADDING an address, which issues a confirmation
// link straight away. Null everywhere else: a live token has no business in a list response, and
// the client only needs it on a machine with no mail server, where it is shown instead.
//
// A plain comment, not a doc comment: /// inside a positional record's parameter list is not a
// valid place for one (CS1587), and hoisting it to a <param> tag would leave the other ten
// parameters undocumented and trade one warning for several.
