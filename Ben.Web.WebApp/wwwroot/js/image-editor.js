...
export function applyAdjustments(containerId, opts) {
    ...
    const img = inst.baseImage;
    const filters = [];
    if (opts.brightness !== 0) filters.push(new fabric.Image.filters.Brightness({ brightness: opts.brightness / 100 }));
    if (opts.contrast   !== 0) filters.push(new fabric.Image.filters.Contrast({ contrast: opts.contrast / 100 }));
    if (opts.saturation !== 0) filters.push(new fabric.Image.filters.Saturation({ saturation: opts.saturation / 100 }));
    if (opts.hue        !== 0) filters.push(new fabric.Image.filters.HueRotation({ rotation: opts.hue / 360 }));
    if (opts.blur        > 0)  filters.push(new fabric.Image.filters.Blur({ blur: opts.blur / 100 }));
    if (opts.noise       > 0)  filters.push(new fabric.Image.filters.Noise({ noise: opts.noise }));
    img.filters = filters;
    img.applyFilters();
    inst.canvas.renderAll();
    _notifyChanged(containerId);
...
export function bringForward(containerId) {
    const inst = _instances.get(containerId);
    const active = inst?.canvas.getActiveObject();
    if (active && active !== inst.baseImage) { inst.canvas.bringForward(active); inst.canvas.renderAll(); }
}

export function sendBackward(containerId) {
    const inst = _instances.get(containerId);
    const active = inst?.canvas.getActiveObject();
    if (active && active !== inst.baseImage) { inst.canvas.sendBackwards(active); inst.canvas.renderAll(); }
}
...
export function moveLayerUp(containerId, id) {
    const inst = _instances.get(containerId);
    const obj  = _findById(containerId, id);
    if (obj && inst) { inst.canvas.bringForward(obj); inst.canvas.renderAll(); }
}

export function moveLayerDown(containerId, id) {
    const inst = _instances.get(containerId);
    const obj  = _findById(containerId, id);
    if (obj && inst) { inst.canvas.sendBackwards(obj); inst.canvas.renderAll(); }
}
