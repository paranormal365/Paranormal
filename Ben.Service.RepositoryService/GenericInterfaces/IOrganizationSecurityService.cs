...
/// <summary>
/// Provides organisation-level security and membership operations used by the
...
public interface IOrganizationSecurityService
{
    ...
    /// </summary>
    /// <param name="actingUserId">The user performing the search.  SuperAdmins see all users; others see only users who share an active organisation membership.</param>
    ...
    /// on <paramref name="tableName"/> within the organisation.
    /// </summary>
    /// <param name="appUserId">The user whose access is being checked.</param>
    /// <param name="organizationId">The target organisation.</param>
    ...
    /// <param name="token">Propagates cancellation to the database query.</param>
    /// <remarks>
    /// SuperAdmins always return <c>true</c>.  Organisation Owners and Administrators
    ...
    /// <summary>
    /// Returns the list of organisations the user is an active member of.
    /// SuperAdmins receive all organisations.
    /// </summary>
    /// <param name="appUserId">The user whose organisations are requested.</param>
    /// <param name="token">Propagates cancellation to the database query.</param>
    Task<IReadOnlyList<Organization>> GetOrganizationsForUserAsync(Guid appUserId, CancellationToken token = default);

    /// <summary>
    /// Creates a new organisation and adds <paramref name="appUserId"/> as its Owner.
    /// </summary>
    /// <param name="appUserId">The user who will own the new organisation.</param>
    /// <param name="name">Display name of the organisation.  Must not be blank.</param>
    /// <param name="urlName">URL-safe slug for the organisation.  Must be unique across all organisations.</param>
    ...
    /// <summary>
    /// Returns all membership rows for the organisation, ordered by role then creation date.
    /// </summary>
    /// <param name="organizationId">The organisation to query.</param>
    /// <param name="actingUserId">Must be a SuperAdmin or an Owner/Administrator of the organisation; otherwise an <see cref="UnauthorizedAccessException"/> is thrown.</param>
    /// <param name="token">Propagates cancellation to the database query.</param>
    ...
    /// <summary>
    /// Creates or updates the membership row for a user in an organisation.
    /// </summary>
    ...
    /// <param name="actingUserId">Must be a SuperAdmin or an Owner/Administrator of the organisation.</param>
    /// <param name="token">Propagates cancellation to the database write.</param>
    ...
    /// </summary>
    /// <param name="organizationId">Target organisation.</param>
    /// <param name="targetUserId">The user receiving the grant.  Must be an active member of the organisation.</param>
