...
public interface IBenStoreAdminClient
{
    ...
    Task<ItemResult<StoreProductEconomicsRecord>> GetStoreProductEconomicsAsync(Guid productId, CancellationToken token = default);
    ...
    Task<ItemResult<StoreExtrasRecord>> GetStoreProductExtrasAsync(Guid productId, CancellationToken token = default);

    Task<(StoreExtrasRecord? Result, string? Error)> SaveStoreProductExtrasAsync(Guid productId, SaveStoreExtrasRequest request, CancellationToken token = default);

    Task<(StoreExtrasRecord? Result, string? Error)> AddStoreProductVideoAsync(Guid productId, MultipartFormDataContent content, CancellationToken token = default);

    Task<(StoreExtrasRecord? Result, string? Error)> DeleteStoreProductVideoAsync(Guid productId, Guid videoId, CancellationToken token = default);
    ...
    Task<(StoreFaqsRecord? Result, string? Error)> SaveStoreProductFaqsAsync(Guid productId, SaveStoreFaqsRequest request, CancellationToken token = default);

    Task<LoadResult<StoreReceivedQuestionRecord>> GetStoreQuestionsAsync(bool openOnly = false, bool siteStockOnly = false, CancellationToken token = default);

    Task<(StoreReceivedQuestionRecord? Result, string? Error)> AnswerStoreQuestionAsync(Guid questionId, AnswerStoreQuestionRequest request, CancellationToken token = default);

    Task<(StoreReceivedQuestionRecord? Result, string? Error)> PromoteStoreQuestionAsync(Guid questionId, PromoteStoreQuestionRequest request, CancellationToken token = default);
