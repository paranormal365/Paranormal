...
public class JwtClaimsParserTests
{
    ...
    public void ParseClaims_ValidToken_ExtractsUserIdFromSubClaim()
    {
        ...
        var (userId, _, _, _) = JwtClaimsParser.ParseClaims(token);
    ...
    public void ParseClaims_StringRole_SuperAdmin_ReturnsSuperAdmin()
    {
        ...
        var (_, isSuperAdmin, _, _) = JwtClaimsParser.ParseClaims(token);
    ...
    {
        var (userId, isSuperAdmin, _, _) = JwtClaimsParser.ParseClaims("not.a.valid.jwt.token.at.all");
    ...
    {
        var (userId, isSuperAdmin, _, _) = JwtClaimsParser.ParseClaims(string.Empty);
    ...
    {
        var (userId, isSuperAdmin, _, _) = JwtClaimsParser.ParseClaims("onlyone");
    ...
    public void ParseClaims_Base64UrlEncoded_WithDashAndUnderscore_DecodesCorrectly()
    {
        ...
        var (userId, isSuperAdmin, _, _) = JwtClaimsParser.ParseClaims(token);
    ...
    public void ParseClaims_AdminRole_IsReportedSeparatelyFromSuperAdmin()
    {
        ...
        var (_, isSuperAdmin, isAdmin, _) = JwtClaimsParser.ParseClaims(token);
