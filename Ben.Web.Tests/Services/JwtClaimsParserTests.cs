...
public class JwtClaimsParserTests
{
    ...
    public void ParseClaims_ValidToken_ExtractsUserIdFromSubClaim()
    {
        ...
        var (userId, _, _, _, _) = JwtClaimsParser.ParseClaims(token);
    ...
    public void ParseClaims_StringRole_SuperAdmin_ReturnsSuperAdmin()
    {
        ...
        var (_, isSuperAdmin, _, _, _) = JwtClaimsParser.ParseClaims(token);
    ...
    {
        var (userId, isSuperAdmin, _, _, _) = JwtClaimsParser.ParseClaims("not.a.valid.jwt.token.at.all");
    ...
    {
        var (userId, isSuperAdmin, _, _, _) = JwtClaimsParser.ParseClaims(string.Empty);
    ...
    {
        var (userId, isSuperAdmin, _, _, _) = JwtClaimsParser.ParseClaims("onlyone");
    ...
    public void ParseClaims_Base64UrlEncoded_WithDashAndUnderscore_DecodesCorrectly()
    {
        ...
        var (userId, isSuperAdmin, _, _, _) = JwtClaimsParser.ParseClaims(token);
    ...
    public void ParseClaims_AdminRole_IsReportedSeparatelyFromSuperAdmin()
    {
        ...
        var (_, isSuperAdmin, isAdmin, _, _) = JwtClaimsParser.ParseClaims(token);
    ...
    [Fact]
    public void ParseClaims_SellerRole_IsReported_AndSuperAdminIsNotASeller()
    {
        // Store sellers (backlog 251): a seller's workspace is their own items, so SuperAdmin does
        // not imply it the way it implies Moderator (SellerRequirement on the server agrees).
        var (_, _, _, _, seller) = JwtClaimsParser.ParseClaims(MakeJwt("""{"role":["Seller"]}"""));
        var (_, _, _, moderates, superSeller) = JwtClaimsParser.ParseClaims(MakeJwt("""{"role":"SuperAdmin"}"""));

        Assert.True(seller);
        Assert.True(moderates);
        Assert.False(superSeller);
    }
}
